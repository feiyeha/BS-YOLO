...
class EMASlideLoss:
    @staticmethod
    def decay_function(x, decay=0.4, tau=2000):
            return decay * (1 - math.exp(-x / tau))
    def __init__(self, loss_fcn, decay=0.4, tau=2000):
    ...
    def __call__(self, pred, true, auto_iou=0.2):
        if self.is_train and auto_iou != -1:
            self.updates += 1
            d = EMASlideLoss.decay_function(self.updates, self.decay, self.tau)
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        self.bbox_loss = BboxLoss(m.reg_max,self.nwdloss,self.iou_ratio).to(device)
