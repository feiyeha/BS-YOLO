...
def video_detection_and_segmentation(background_mask_path, output_dir):
    ...
    # 初始化视频写入器（用于保存掩码图视频）
    mask_output_video_path = os.path.join(output_dir, "mask_output_video.mp4")
    mask_out = cv2.VideoWriter(mask_output_video_path, fourcc, fps, (frame_width, frame_height))
    ...
    while True:
        ...
        for detection, category_id in zip(detection_boxes, category_ids):
            ...
            if count==frame_count:
                ...
                count=count+1
                # 将掩码图转换为彩色图像
            mask_colored = cv2.cvtColor(blind_way_mask, cv2.COLOR_GRAY2BGR)
                #同一帧的长宽不需要变，同一帧的掩码图和背景掩码图也不用变
            violation, violation_box=is_parking_violation(detection, category_id, blind_way_mask, blind_way_mask2, frame_height, frame_width)
            if violation:
                violation_found = True
                violation_boxes.append(violation_box)
                # 绘制违停框（红色）
                cv2.rectangle(segimg, violation_box[:2], violation_box[2:], (0, 0, 255), 2)
                cv2.rectangle(mask_colored, violation_box[:2], violation_box[2:], (0, 0, 255), 2)
            else:
                # 绘制正常检测框（绿色）
                cv2.rectangle(segimg, violation_box[:2], violation_box[2:], (0, 255, 0), 2)
                cv2.rectangle(mask_colored, violation_box[:2], violation_box[2:], (0, 255, 0), 2)
        # 如果当前帧有违停，保存违停帧
        if violation_found:
        ...
        out.write(segimg)
        # 显示绘制后的掩码图
        cv2.imshow("Mask with Boxes", mask_colored)

        # 将绘制后的掩码图写入视频
        mask_out.write(mask_colored)
            # 控制播放速度，按 'q' 键退出
