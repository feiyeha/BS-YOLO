...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, labels):
        ...
        for i in range(len(self.matrix)):  
            ...
            row_sum = 0
            every_cate_sum=0
            # 遍历当前行的所有元素（注意，这里j从0开始，但我们会跳过i==j的情况）  
            for j in range(len(self.matrix[i])):  
        # 如果不是主对角线上的元素，则加到当前行的和中  
                if i != j:  
                    row_sum += self.matrix[i][j] 
                every_cate_sum+=self.matrix[i][j]
    # 将当前行的和添加到列表中  
            row_sums.append(row_sum)
            every_cate_sums.append(every_cate_sum)
        
        column=[]
        column_sums=[]
        for j in range(len(self.matrix[0])):  # 假设矩阵至少有一行  
        # 初始化当前列的和为0  
            column_sum = 0  
            zongcolumn_sum=0
            # 遍历矩阵的每一行  
            for i in range(len(self.matrix)):  
            # 如果不是主对角线上的元素，则加到当前列的和中  
                if i != j:  
                    column_sum += self.matrix[i][j]
                zongcolumn_sum+=self.matrix[i][j]
            # 将当前列的和添加到列表中  
            column.append(column_sum) 
            column_sums.append(zongcolumn_sum) 
        # 跑验证的时候开，巡礼的时候由于数据不够不能跑
        self.lou = sum(self.matrix[-1, :])
        self.row=row_sums
        self.every_sums=every_cate_sums
        self.column=column
