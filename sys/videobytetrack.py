...
from VehicleTimer import VehicleTimer
...
def video_detection_and_segmentation(background_mask_path, output_dir):
    ...
    vehicle_timer = VehicleTimer(violation_threshold=10.0, iou_threshold=0.7)
    ...
    while True:
        ...
        for i,(detection, category_id) in enumerate(zip(detection_boxes, category_ids)):
            ...
            if violation and track_id is not None:
                elapsed_time, is_violation = vehicle_timer.update(track_id, violation_box)
                if is_violation:
            ...
            if track_id is not None:
                x, y, w, h = detection
                center = (int(x), int(y))  # 获取目标中心点
                track = track_history[track_id]
                track.append(center)  # 将中心点添加到轨迹中
                if len(track) > 20:  # 保留最近的20个点
                    track.pop(0)

                # 绘制轨迹线
                points = np.array(track, dtype=np.int32).reshape((-1, 1, 2))
                cv2.polylines(segimg, [points], isClosed=False, color=(0, 255, 255), thickness=2)
            # 在框的左上角显示跟踪ID和置信度
