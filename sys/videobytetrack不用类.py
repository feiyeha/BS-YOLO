...
from VehicleTimer import VehicleTimer,calculate_iou
...
def video_detection_and_segmentation(background_mask_path, output_dir):
    ...
    if os.path.exists(out):
        shutil.rmtree(out)
    # 检测视频路径
    video_path = "generate background.mp4"
    # 加载语义分割模型
    classes = 1  # exclude background
    weights_path = "weights/model_best.pth"
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    # 用于存储每个车辆的计时信息,可以用下面的计时器类替换
    vehicle_timers = defaultdict(lambda: {
        "start_time": None,  # 违停开始时间
        "initial_box": None,  # 初始检测框
        "current_box": None,  # 当前检测框
    })
    violation_threshold = 10.0  # 违停时间阈值（秒）
    iou_threshold = 0.7  # IoU 阈值，用于判断车辆是否移动
    ...
    while True:
        ...
        for i,(detection, category_id) in enumerate(zip(detection_boxes, category_ids)):
            ...
            if violation:
                if vehicle_timers[track_id]["start_time"] is None:
                    # 首次判定为违停，记录开始时间和初始检测框
                    vehicle_timers[track_id]["start_time"] = time.time()
                    vehicle_timers[track_id]["initial_box"] = violation_box
                    vehicle_timers[track_id]["current_box"] = violation_box
                else:
                    # 更新当前检测框
                    vehicle_timers[track_id]["current_box"] = violation_box
                    # 计算当前检测框与初始检测框的 IoU
                    iou = calculate_iou(vehicle_timers[track_id]["initial_box"],
                                        vehicle_timers[track_id]["current_box"])
                    ...
                    # 检查是否超过违停时间阈值
                    elapsed_time = time.time() - vehicle_timers[track_id]["start_time"]
                    if elapsed_time >= violation_threshold:
                        print(f"Vehicle {track_id} has parked illegally for {elapsed_time:.2f} seconds!")
                        # 保存违停帧
                        violation_frame_path = os.path.join(output_dir, f"longtimeviolation_car_{track_id}.jpg")
                ...
                text_color=(0, 0, 255)
            #     在判断为违停的时候，开始记录时间，但不能重复刷新记录，当前记录后开始计时，如果已经开始计时了就不刷新，继续算，
            # 计算10s的坐标变化
            # 10s后汽车如果还是用盲道像素点判断违停（说不定10s内移动还是没有太大变化）或者记录当时的IOU然后和10s后的对比IOU
            else:
                if track_id is not None:
                    vehicle_timers[track_id]["start_time"] = None
                    vehicle_timers[track_id]["initial_box"] = None
                    vehicle_timers[track_id]["current_box"] = None
                # 绘制正常检测框（绿色）
